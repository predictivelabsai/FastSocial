#!/usr/bin/env python3
"""Run FastSocial deployment commands through the sibling FastDevOps control plane."""
...
service = next(
    (name for name, spec in catalog().items() if spec.get("local_dir") == ROOT.name), None
)
if not service:
...
command_name, *options = sys.argv[1:]
sys.argv = [
    sys.argv[0],
    command_name,
    *([] if command_name == "validate" else [service]),
    *options,
]
main()
